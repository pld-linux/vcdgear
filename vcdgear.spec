...
License:	unknown
Source0:	http://www.vcdgear.com/???/vcd-%{version}.tar.gz
...
%description
This program allow convert MPEG files in various format: .cue --> .raw
(.cue to .raw/.bin track image), .cue --> .dat, .cue --> .mpg, .raw -->
.dat (.raw/.bin track image to .dat), .raw --> .mpg, .cif --> .mpg,
.mpg --> .mpg, .mpg --> .dat, .dat --> .mpg.
...
install %{name}15	$RPM_BUILD_ROOT%{_bindir}/%{name}
